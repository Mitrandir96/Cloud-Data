...
public class ExceptionHandlerAdvice {
    ...
    public ResponseEntity<String> onLoginError (LoginException e) {
        return prepareBadRequestResponseEntity(e.getMessage(), 1, HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<String> onLoginValidationError(MethodArgumentNotValidException e) {
        return prepareBadRequestResponseEntity(e.getFieldError().getDefaultMessage(), 2, HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(AuthException.class)
    public ResponseEntity<String> authenticationError(AuthException e) {
        return prepareBadRequestResponseEntity(e.getMessage(), 3, HttpStatus.UNAUTHORIZED);
    }

    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<String> onFileValidationError(IllegalArgumentException e) {
        return prepareBadRequestResponseEntity(e.getMessage(), 4, HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(IOException.class)
    public ResponseEntity<String> getFileBytesError(IOException e) {
        return prepareBadRequestResponseEntity(e.getMessage(), 5, HttpStatus.BAD_REQUEST);
    }

    private ResponseEntity<String> prepareBadRequestResponseEntity(String message, int id, HttpStatus status) {
        var gson = new Gson();
